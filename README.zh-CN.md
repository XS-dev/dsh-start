...
在希望 Harness 操作的项目目录中运行：

```bash
dsh
```

默认端口是 **3080**。终端显示以下内容后：

```text
dsh web: http://127.0.0.1:3080
```

在浏览器中打开 [http://127.0.0.1:3080](http://127.0.0.1:3080)。使用 Web UI 期间需要保持终端运行；不再使用时，在终端按 `Ctrl+C` 停止服务。

默认情况下，服务只监听本机地址。如果指定了其他端口，请打开对应端口的地址。例如执行 `dsh --port 8080` 后，访问 [http://127.0.0.1:8080](http://127.0.0.1:8080)。
